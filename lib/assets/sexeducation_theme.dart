import 'package:flutter/material.dart';

const lightColorScheme = ColorScheme(
  brightness: Brightness.light,
  primary: Color(0xFFD1EC3F),
  onPrimary: Color(0xFFFFFFFF),
  primaryContainer: Color(0xFFFFD1BD),
  onPrimaryContainer: Color(0xFF00210C),
  secondary: Color(0xFF506352),
  onSecondary: Color(0xFFFFFFFF),
  secondaryContainer: Color(0xFFD3E8D2),
  onSecondaryContainer: Color(0xFF0E1F12),
  tertiary: Color(0xFF3A656E),
  onTertiary: Color(0xFFFFFFFF),
  tertiaryContainer: Color(0xFFBDEAF5),
  onTertiaryContainer: Color(0xFF001F25),
  error: Color(0xFFBA1A1A),
  errorContainer: Color(0xFFFFDAD6),
  onError: Color(0xFFFFFFFF),
  onErrorContainer: Color(0xFF410002),
  background: Color(0xFFFCFDF7),
  onBackground: Color(0xFF191C19),
  outline: Color(0xFFE5BDFF),
  onInverseSurface: Color(0xFFF0F1EC),
  inverseSurface: Color(0xFF2E312E),
  inversePrimary: Color(0xFF1EE377),
  shadow: Color(0xFF000000),
  surfaceTint: Color(0xFFFFD1BD),
  outlineVariant: Color(0xFFC1C9BF),
  scrim: Color(0xFF000000),
  surface: Color(0xFFEBEFDA),
  onSurface: Color(0xFF191C19),
  surfaceVariant: Color(0xFFDDE5DA),
  onSurfaceVariant: Color(0xFFFFD1BD),
);

const darkColorScheme = ColorScheme(
  brightness: Brightness.dark,
  primary: Color(0xFFE5BDFF),
  onPrimary: Color(0xFF003918),
  primaryContainer: Color(0xFF005226),
  onPrimaryContainer: Color(0xFF62FF96),
  secondary: Color(0xFFB7CCB7),
  onSecondary: Color(0xFF233426),
  secondaryContainer: Color(0xFF394B3B),
  onSecondaryContainer: Color(0xFFD3E8D2),
  tertiary: Color(0xFFA2CED8),
  onTertiary: Color(0xFF01363E),
  tertiaryContainer: Color(0xFF204D56),
  onTertiaryContainer: Color(0xFFBDEAF5),
  error: Color(0xFFFFB4AB),
  errorContainer: Color(0xFF93000A),
  onError: Color(0xFF690005),
  onErrorContainer: Color(0xFFFFDAD6),
  background: Color(0xFF191C19),
  onBackground: Color(0xFFE2E3DE),
  outline: Color(0xFFD1EC3F),
  onInverseSurface: Color(0xFF191C19),
  outlineVariant: Color(0xFFC1C9BF),
  inverseSurface: Color(0xFFE2E3DE),
  inversePrimary: Color(0xFF006D35),
  shadow: Color(0xFF000000),
  surfaceTint: Color(0xFF1EE377),
  scrim: Color(0xFF000000),
  surface: Color(0xFF111411),
  onSurface: Color(0xFFC5C7C2),
  surfaceVariant: Color(0xFF414941),
  onSurfaceVariant: Color(0xFFFFD1BD),
);